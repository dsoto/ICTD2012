...
electronics.
Figure \ref{ShedWiringDiagram} shows the power flow between the generation and
...
The structure is approximately two meters by two meters by three meters tall
...
The inverter supplies power to the system and the customers at 220 V and
50 Hz.
...
cabinets.
This control enclosure contains communications hardware and a
plug computer.
...
A Linux plug computer, (SheevaPlug) which is connected to the switch,
...
more robust than our custom solution by housing it in a more robust
location with more reliable power and communications.
...
Scratch cards are available locally to in
amounts as low as \$1 USD (Figure \ref{scratchCards}) and up to \$4 USD.
The cards contain instructions on how to recharge the account
and a concealed authorization code.
The customer enters the revealed code into his mobile phone and sends by
...
When credit level reaches a low setpoint, a message is sent to the
gateway server, which in turn sends a message to the consumer warning
him that his account is low and should be refilled soon.
% inquiry
...
purchasing of mobile phone airtime, we conducted trainings \ref{training}
...
Figure \ref{scatterCreditTime} plots consumption against the fraction of time
with available credit and
...
and cannot measure loads below 1.5 W and consume 1.5 W--2.5 W.  These
...
cellphone charger can consume 12 Wh in a day, equivalent to 1--2 hours
...
We are also developing custom electronics that have lower power consumption
